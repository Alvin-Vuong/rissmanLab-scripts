...
% Initialize paths
save_dir = '/space/raid6/data/rissman/Nicco/NIQ/EXPANSION/Probtrack_Subject_Specific/Compiled_Values/';
...
for s = 1:8 %length(subjs)
    ...
    % Init matrices
    mean_non_zero = zeros(264,264);
    voxels_non_zero = zeros(264,264);
    volume_non_zero = zeros(264,264);
    
    %count_non_zero = zeros(264,264);
    %mean_non_zero_div_waytotal = zeros(264,264);
    %count_non_zero_div_waytotal = zeros(264,264);
    ...
    parfor seed = 1:264
        ...
        cd([top_dir subject_str '/From_' seed_str]);
        ...
        temp_mean_non_zero = zeros(1,264);
        temp_voxels_non_zero = zeros(1,264);
        temp_volume_non_zero = zeros(1,264);
        
        %temp_count_non_zero = zeros(1,264);
        %temp_mean_non_zero_div_waytotal = zeros(1,264);
        %temp_count_non_zero_div_waytotal = zeros(1,264);
        ...
        for t = 1:length(targets)
            ...
            filename = ['seeds_to_' subject_str '_Petersen_' target_str '.nii.gz'];
            fprintf('Target: %s\n', target_str);
    ...
    save([save_dir 'Subj_' subject_str '.mat'],'mean_non_zero','voxels_non_zero','volume_non_zero');
